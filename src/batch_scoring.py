...
def main():
    ...
    parser.add_argument("--method", type=str, required=True, choices=["MM", "PLL", "LLR", "MLLR", "EDS", "EnsembleMLLR", "EntropyMLLR", "MutantMarginal"], help="Scoring method.")
    ...
    parser.add_argument("--reference_csv", type=str, default="data/ProteinGym_reference_file_substitutions.csv", help="Path to reference CSV for WT sequence.")
    parser.add_argument("--max_samples", type=int, default=None, help="Stop after processing N samples (for testing).")
    parser.add_argument("--wt_sequence", type=str, default=None, help="Manually provide WT sequence (for EDS).")
    ...
    # Wrap entire execution in try-except to catch silent failures
    try:
        # 1. Load Data
        logging.info(f"Loading data from {args.input_csv}...")
        print(f"DEBUG: Loading data from {args.input_csv}...", flush=True)
        ...
        if has_mutated_seq:
            ...
            wt_seq = args.wt_sequence # Use manual arg if provided
            
            # Try to find WT in the dataframe if not provided
            if wt_seq is None:
                wt_rows = df[df['mutant'] == 'WT']
                if not wt_rows.empty:
                    wt_seq = wt_rows.iloc[0]['mutated_sequence']
                    logging.info(f"Found WT sequence in input CSV: {wt_seq[:10]}...")
        else:
        ...
        elif args.method == "EDS":
            scorer = EmbeddingDistanceScoring()
        elif args.method == "EnsembleMLLR":
             scorer = EnsembleMaskedLogLikelihoodRatioScoring()
        elif args.method == "EntropyMLLR":
             scorer = EntropyCorrectedLogLikelihoodRatioScoring()
        elif args.method == "MutantMarginal":
             scorer = MutantMarginalScoring()
        else:
        ...
        print(f"DEBUG: Starting scoring loop...", flush=True)
        ...
        with open(args.output_csv, mode, buffering=1) as f:
            ...
            for i in tqdm(range(start_idx, end_idx)):
                ...
                if i == start_idx:
                    logging.info(f"DEBUG: First Mutant Code: {mutant_code}")
                    logging.info(f"DEBUG: WT Seq (first 10): {current_wt[:10]}")
                    logging.info(f"DEBUG: Mut Seq (first 10): {mut_seq[:10]}")
                    if current_wt == mut_seq and mutant_code != "WT":
                        logging.error("CRITICAL: WT and Mutant sequences are IDENTICAL! Mutation application failed.")
                    else:
                        logging.info("DEBUG: Sequences differ. Mutation applied successfully.") 
                ...
                try:
                    ...
                    try:
                        score_val = scorer.score(model_wrapper, current_wt, mutant_code, mut_seq, batch_size=args.batch_size)
