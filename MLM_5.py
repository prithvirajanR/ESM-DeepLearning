...
df_double["LLR"] = df_double["sequence"].apply(lambda x: llr_score(wt_seq, x, tokenizer, model))
...
df_clean = df_double.dropna(subset=['fitness', 'LLR'])
...
plt.ylabel('ESM-2 Log-Likelihood Ratio (Double Mutation)', fontsize=12)
...
plt.tight_layout()
plt.savefig("llr_correlation_plot_double_mutation.png", dpi=300)
