...
df_single["LLR"] = df_single["sequence"].apply(lambda x: llr_score(wt_seq, x, tokenizer, model))
...
df_clean = df_single.dropna(subset=['fitness', 'LLR'])
df_clean = df_clean[~np.isinf(df_clean['LLR'])]


# In[ ]:


corr, pval = scipy.stats.spearmanr(df_clean['fitness'], df_clean['LLR'])
...
plt.scatter(df_clean['fitness'], df_clean['LLR'], alpha=0.5, s=10, c='blue', label='Mutants')


# In[ ]:


z = np.polyfit(df_clean['fitness'], df_clean['LLR'], 1)
p = np.poly1d(z)
x_range = np.linspace(df_clean['fitness'].min(), df_clean['fitness'].max(), 100)
plt.plot(x_range, p(x_range), "r--", linewidth=2, label=f'Trend ({corr:.3f})')
...
plt.ylabel('ESM-2 Log-Likelihood Ratio (Single Mutation)', fontsize=12)
plt.title(f'Fitness vs. LLR Score\nSpearman Correlation = {corr:.3f}', fontsize=14)
...
plt.tight_layout()
plt.savefig("llr_correlation_plot_single_mutation.png", dpi=300)
